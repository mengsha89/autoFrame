...
class TestDtStartClass(unittest.TestCase):
    """
    综合场景（普通提问、PK提问、抢红包、下课提问）
    """
    @classmethod
    def setUpClass(cls):
    ...
    def test_002_snatch_red_envelope(self):
        """
        抢红包
        """
        def alive():
    ...
    def test_003_pk_answer(self):
        ...
        for th in threadpool:
            th.start()
            th.join()
        save_answer(self.main_class_id, self.lesson_id, correct_answer, score, topic_id, topic_sub_id, topic_no,
                    topic_sub_no,
                    type_=0, answer_type=2)
    ...
    def test_004_ask_class_problem(self):
        """
        下课提问
        """
        def alive():
        ...
        t.setDaemon(True)
        t.start()
        start_class_problem(self.main_class_id, self.lesson_id)
        bus_id = "lessonId:%s" % self.lesson_id
        update_status_and_vali(1, 32, bus_id, bus_id)
        revoke_class_problem(self.main_class_id, self.lesson_id)
        update_status_and_vali(32, 1, bus_id, bus_id)

...
class TestDtAskNorMouQues(unittest.TestCase):
    ...
    @data(*dt)
    def test_001_answer_question(self, topic_id):
        def alive():
            while True:
                keep_alive()
                time.sleep(10)
        ...
        start_answer(self.main_class_id, self.lesson_id, correct_answer, topic_id=topic_id, type_=0, answer_type=1)
        ...
        # top_stus = str(top_stus).replace(r'u\'', r'\'').decode("unicode-escape")
        # print "top_stus:%s" % top_stus
